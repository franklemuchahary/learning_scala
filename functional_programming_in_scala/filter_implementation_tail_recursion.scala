...
object Solution {
	...
	def filter_tail(delim:Int, arr:List[Int], res:List[Int]):List[Int] = {
	    /// tail recursion implementation of the above function -> scala automatically optimizes this to use only one stack frame
	    arr match {
	        case arr if arr.length == 0 => 
	            res
	        case arr if arr.head < delim => 
	            filter_tail(delim, arr.tail, res ++ List[Int](arr.head))
	        case arr => 
	            filter_tail(delim, arr.tail, res)
	    }
	}
	...
	def main(args : Array[String]){
		...
		var result = filter_tail(num, arr, List[Int]())	
		println(result)
	}
}
